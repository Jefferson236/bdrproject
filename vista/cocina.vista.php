...
<head>
    ...
    <style>
        ...
        .ticket-header { padding: 15px; text-align: center; color: #fff; }
        .bg-pendiente { background-color: #d62300; }
        .bg-preparando { background-color: #ff9800; }
        .cliente-name { display: block; font-size: 1.2rem; font-weight: bold; color: #fff; text-transform: uppercase; border-bottom: 1px solid rgba(255,255,255,0.3); margin-bottom: 5px; padding-bottom: 5px; }
        ...
        .btn-empezar { background-color: #ff9800; color: #000; } 
        .btn-terminar { background-color: #4caf50; color: #fff; }
...
<body>
    <div class="navbar-kds">
        <div><h2 style="margin:0; color: #d62300;">🔥 KITCHEN DISPLAY SYSTEM</h2><small style="color: #aaa;">Actualización cada 15s</small></div>
        <div>
            <span class="me-3">Chef: <?= htmlspecialchars($_SESSION['username'] ?? 'Personal') ?></span>
                ...
                <div class="col-md-4 col-lg-3">
                    <div class="ticket">
                        <div class="ticket-header <?= $pedido['estado'] == 'pendiente' ? 'bg-pendiente' : 'bg-preparando' ?>">
                            <span class="cliente-name">👤 <?= htmlspecialchars($pedido['cliente']) ?></span>
                            TICKET #00<?= $id ?><br>
                            <span class="time-badge">🕒 <?= date('H:i', strtotime($pedido['fecha'])) ?></span>
                        </div>
                        ...
                        <div class="ticket-footer">
                            <form method="POST">
                                ...
                                <?php if($pedido['estado'] == 'pendiente'): ?>
                                    ...
                                    <button type="submit" name="cambiar_estado" class="btn-kds btn-empezar">👨‍🍳 Empezar</button>
                                <?php else: ?>
                                    <input type="hidden" name="nuevo_estado" value="listo">
                                    <button type="submit" name="cambiar_estado" class="btn-kds btn-terminar">✅ Listo</button>
                                <?php endif; ?>
                            </form>
                        </div>
                    </div>
